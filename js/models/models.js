define('models', ['moment', 'seeker', 'jquery', 'logger'], function(m, seeker, $, logger) {
	...
	Model.prototype.parseImages = function(searchResult) {
		if (searchResult.image) {
			this.images = {};
			searchResult.image.forEach(function(imgData) {
				this.images[imgData.size] = imgData['#text'];
			}, this);
		}
	};
	...
	function Album(searchResult) {
		...
		if (searchResult.releasedate && searchResult.releasedate.length > 4) {
			this.releaseDate = moment($.trim(searchResult.releasedate), 'D MMM YYYY, HH:mm'); // 31 May 1994, 00:00 
			this.releaseYear = this.releaseDate.format('YYYY');
		}
		if (searchResult.tracks) {
			var albumTracks = [],
				rawTracks = ($.isArray(searchResult.tracks.track) ? searchResult.tracks.track : [searchResult.tracks.track]);
	...
	*/
	...
	function Artist(searchResult) {
		...
		if (searchResult) {
			...
			// this.externalId = (this.externalId) ? this.externalId : 'lfm|#|' + this.name;
		}
	}
